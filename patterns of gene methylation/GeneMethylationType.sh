...
while read LIB_LINE; do
   ...
   awk '{if (($6>='"${cCG}"') && ($8>='"${cCHG}"') && ($5<='"${mCG1}"') && ($7<='"${mCHG1}"')) {print $0,"UM"} \
   else if (($6>='"${cCG}"') && ($8>='"${cCHG}"') && ($5>='"${mCG2}"') && ($7<='"${mCHG1}"')) {print $0,"gbM"} \
   else if (($6>='"${cCG}"') && ($8>='"${cCHG}"') && ($5>='"${mCG3}"') && ($7>='"${mCHG3}"')) {print $0,"teM"} \
   else {print $0,"ambiguous"}}' OFS="\t" ${genome}.cgchg.mtr > ${genome}.gene.mtr.ID.type
