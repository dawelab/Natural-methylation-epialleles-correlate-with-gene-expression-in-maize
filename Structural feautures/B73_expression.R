...
#The abbrevation of MP, MS, SC & TP are Mature pollen, Microspore, Sperm Cell, Tassel Primodial,respectively.
#Warman_MP = read.table("/Users/x/Desktop/Data/expression/MP.merged.bam.count",header = T)
#Warman_MS = read.table("/Users/x/Desktop/Data/expression/MS.merged.bam.count",header = T)
#Warman_SC = read.table("/Users/x/Desktop/Data/expression/SC.merged.bam.count",header = T)
#Warman_TP = read.table("/Users/x/Desktop/Data/expression/TP.merged.bam.count",header = T)
...
B73.core.epi <- merge(B73.core,B73.epiallele,by="gene",all.x = T)[,-2] %>% filter(epiallele!="ambiguous") %>% unique()

temp = merge(Hufford_data,B73.core.epi,by="gene",all.y = T) 
...
df_pattern$methylation = factor(df_pattern$methylation,levels = c("teM","gbM","UM"))
df_pattern$pattern = factor(df_pattern$pattern,
                            levels = c("silenced","expressed","tissue"))
